...
@php
    $count = count($subjectCards);

    $gridCols = $count <= 4 ? 'lg:grid-cols-2' : 'lg:grid-cols-3';
    $maxWidth = $count <= 4 ? 'max-w-4xl' : 'max-w-7xl';
@endphp
            ...
            <div class="w-full {{ $maxWidth }}">
                <div class="grid grid-cols-1 md:grid-cols-2 {{ $gridCols }} gap-4">
                    ...
                    @foreach ($subjectCards as $item)
                        <div
                            class="{{ $item['bg'] }} text-background shadow-xl rounded-3xl py-5 pl-4 pr-6 min-h-[300px]">
